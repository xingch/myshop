...
class buyModel {
    ...
    public function createOrder($input, $member_id, $member_name, $member_email) {
		...
		$reciver_info['post_code'] = $address_info['post_code'];
        $reciver_info['id_number'] = $address_info['id_number'];
	...
	public function makeOrderSn($pay_id) {
	    ...
	    if (empty($num)) {
		 ...
		 return 'HGT' . (date('y',time()) % 9+1) . sprintf('%013d', $pay_id) . sprintf('%02d', $num);
		// liquan return (date('y',time()) % 9+1) . sprintf('%013d', $pay_id) . sprintf('%02d', $num);
	}
