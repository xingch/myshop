...
$html_footer = <<<EOF
<div class="footer">
  <h6><a href="http://www.higoton.com" target="_blank"></a></h6>
</div>
EOF;
...
function step3(&$install_error,&$install_recover){
	...
	if(mysqli_get_server_info($link) > '5.0') {
		mysqli_query($link,"CREATE DATABASE IF NOT EXISTS `$db_name` DEFAULT CHARACTER SET ".DBCHARSET);
	} else {
		$install_error = '数据库必须为MySQL5.0版本以上';return;
	}
	if(mysqli_errno($link)) {
		$install_error = mysqli_error();return ;
	}
	if($_POST['install_recover'] != 'yes' && ($query = mysqli_query($link,"SHOW TABLES FROM $db_name"))) {
